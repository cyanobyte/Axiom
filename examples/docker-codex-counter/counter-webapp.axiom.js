/**
 * Purpose: Provide the manual Docker live-smoke example for Codex CLI builds.
 * Responsibilities:
 * - Run the live Codex CLI agents from inside the Docker build runner.
 * - Keep generated files isolated under the Docker Codex example workspace.
 * - Serve as the manual end-to-end acceptance target for live Docker builds.
 */
import {
  intent,
  must,
...
export default intent(
  {
    id: "counter-webapp-docker-codex-live",
    meta: {
      title: "Docker Codex Counter Web App Live Smoke",
      summary: "A live-smoke counter app example that runs Codex CLI inside Docker.",
      version: "1.0.0",
      tags: ["example", "docker", "codex", "live", "webapp", "counter"]
    },
    what: {
    ...
    assumptions: [
      ...
      "The host Codex CLI session is already authenticated and mounted read-only into the Docker runner.",
  ...
  async (ctx) => {
    ...
    return {
      ok: true,
      app: "counter-webapp-docker-codex-live",
